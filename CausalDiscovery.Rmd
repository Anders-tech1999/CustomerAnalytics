...
```
The hill-climbing algorithm is a search-based method for learning the structure of a Bayesian Network. It starts with an initial network (often empty) and iteratively makes small changes (adding, deleting, or reversing edges) to the network structure to improve a given scoring criterion (in this case, BIC). The algorithm continues until no further improvements can be made. It is a greedy algorithm that seeks to find a local optimum in the search space of network structures.
...
```
Output: 
 - if removing Plea -> Comm, BIC will decrease with -668.211 
 - which in bnlearn means the model will get worse.
 - if removing Atti -> Comm, BIC will increase with 40.48, 
 - which in bnlearn package means the model may improve based on this index.
 bnlearn reverses the meaning of BIC...?

Plea → Comm:
 - Strength: -668.21187
Interpretation: 
 - This arc shows a strong negative association, which might suggest that 
 - including this relationship in the model could be more harmful than beneficial. It could be a candidate for removal.
...
```
AIC:
bn.gs1: -11842.95       Is closest to zero
bn.hc:  -11919.77

BIC
bn.gs1: -12090.61
bn.hc:  -12050.12       Is closest to zero

BDE
bn.gs1: -11904.39       Is closest to zero
bn.hc:  -11983.02       
...
predComm = predict(
  ...
  predictors = names (retention_test)[-4],#target variable is the 4th column in the testing dataset
  type = "distribution") 
predComm = predComm$pred$Comm
#predComm : Ugly output
```
...
Below we consider several hypothetical situations.   
 ...
 - setting evidence in the network for Fuse and Plea and we´ll look at the cpt for Atti and Comm before and after setting the evidence.  - Setting (hard) evidence means setting one of the states of the variable at probability 1 (100%).
...
Predict with Evidence
 - Imagine a new customer joins the VC reporting a Low (Medium or High) Functional Usefulness perception. 
...
Atti:
    ...
    Low: 0.01905778
    Med: 0.82340889
 - These values represent the probabilities of the node "Atti" being in the states "High", "Low", "Med" 
...
df1 <- data.frame(
  Fuse = c("Low", "Med", "High"), 
  AttiLow, AttiMed, AttiHigh)
df1
matplot(rownames(df1), 
        df1, type='l', xlab='Fuse', ylab='', ylim=c(0,1))
