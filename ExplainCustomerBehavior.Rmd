...
scoreitems$alpha
factordata <- as.data.frame(scoreitems$scores)
factordata
```
Cronbach's Alpha: 
relaxation: 0.92
...
mi :
 ...
 - Higher values suggest more significant potential improvement.
 
 epc:
 - Represents the approximate estimate of how much the parameter would change if it were estimated rather than fixed to zero
 - e.g. the epc value of 0.569 between "Appearance" and "muscle" indicates a substantial expected change.
 - Positive epc values indicate the parameter being positive if it were estimated.
 - Negative epc values indicate the parameter being negative if it were estimated.
 - e.g. an epc of -0.284 for the covariance between "Attractive" and "lweight" suggests a negative relationship between these variables.


##Modified CFA Confirmatory Factor Analysis
```{r Modified CFA Confirmatory Factor Analysis}
...
# acceptable fit
```
RMSEA (modified model): 0.107 which is adequate
...
- minimum accpetable level: 0.5 (look in Estimate column)
    *below 0.50 are candidates for deletion
- p-values significance (all has to be > 0.05)
Latent Variables:
                   Estimate  Std.Err  z-value  P(>|z|)
  Attractive =~                                       
    face              1.000                           
    sexy              1.418    0.376    3.769    0.000
  Appearance =~                                       
    body              1.000                           
    appear            1.322    0.078   16.924    0.000
    attract           1.252    0.081   15.505    0.000
  Muscle =~                                           
    muscle            1.000                           
    strength          0.530    0.093    5.673    0.000
    endur             0.510    0.076    6.737    0.000
  Weight =~                                           
    lweight           1.000                           
    calories          0.995    0.060   16.463    0.000
    cweight           0.977    0.062   15.875    0.000
...
-	Once a satisfactory measurement model is obtained, the 2nd step is to test the structural model (stages 5-6). 
 - The structural model tests a theory and relates the constructs to other constructs
...
# check fit
```
THE STRUCTURAL model fit is assessed as was the CFA model fit
...
- when having adequate level of RMSEA, Tucker-Lewis Index,  Comparative Fit Index and SRMR the model proceeds : Aline Regression output
...
Edges (Arrows):
 - Single-headed Arrows: Indicate causal relationships or regressions. An arrow to Appearance from Attractive suggests that Appearance is predicted by Attractive.
...
x15 does not share much with the other variables. x6 does share 87% with other variables. 
 ...
 - MSA approx 0.3 is critical value (x15 - deletion material)
...
According to the guidelines:
 ...
 - The chi-square test is highly significant (p-value is 1.83e-19) meaning poor fit. But this test is not always reliable because it is highly influenced by the sample size analyzed. 
...
- the threshold is RMSEA < 0.8
- The structural model is a good fit!
...
```
THIS IS AN ADEQUATE SEM OUTPUT IF ASKED
...
SUMMARY:
 - Customers perceptions about CustomerService, ProductValue and MarKeting are positively and significantly correlated with satisfaction. 
...
modificationindices(fitSEM1, sort = T, minimum.value = 3.84, op = "~~")
...
- the x19 could be accused of having a crossload on x6 and x13r
...
components: 
(1) the target constructs of interest – namely, CUSA and CUSL (endogenous constructs) 
 – and 
 (2) the two corporate reputation dimensions COMP and LIKE (exogenous constructs), 
...
simple_mm <- constructs(
  composite("COMP",multi_items("comp_",1:3),weights = mode_A), #reflective: modeA
  composite("LIKE",multi_items("like_",1:3),weights = mode_A), #reflective: modeA
  composite("CUSA",single_item("cusa")), #constructed alone
  composite("CUSL",multi_items("cusl_",1:3),weights = mode_A)) #reflective: modeA
#Mode_A or correlation_weights for reflectively specified measurement models and mode_B or regression_weights for formatively specified measurement models. 
#Mode_A is the default
...
non-parametric means that the method does not make strong assumptions about the distribution of the data. Specifically, it implies the following:
 - No Assumption of Normality:
PLS-SEM does not assume that the data are normally distributed. Many parametric methods, like covariance-based SEM (CB-SEM), rely on the assumption that the data follow a normal distribution, which is not required for PLS-SEM.
 - Flexibility with Data Characteristics:
Because it does not assume normality, PLS-SEM can be used with data that are non-normally distributed, skewed, or have outliers. This makes PLS-SEM particularly useful for real-world data that often violate parametric assumptions.
 - Bootstrapping:
Since PLS-SEM is non-parametric, it does not provide standard errors, t-values, or confidence intervals based on the assumption of a specific data distribution. Therefore, bootstrapping is used to estimate these statistics.
...
#summary(corp_rep_pls_model) hidden beacuse: NOT IMPORTANT IN REFLECTIVE MODEL

Path Coefficients: NOT IMPORTANT IN REFLECTIVE MODEL
        CUSA  CUSL
R^2    0.295 0.562
AdjR^2 0.290 0.558
 - R^2: Indicates the proportion of variance in the dependent variables explained by the independent variables.
...
NOT IMPORTANT IN REFLECTIVE MODEL:
        CUSA  CUSL
COMP   0.162 0.009
LIKE   0.424 0.342
CUSA       . 0.504
...
```
Iterations - NOT IMPORTANT IN REFLECTIVE MODEL
- 4 means that the algorithm converged after 4 iterations.

o	INDICATOR RELIABILITY
 – where we assess if the construct explains more variance of the indicator compared to what is left in the error term
-	High loadings indicate that different measures of a construct have much in common
-	The squared loading of a standardized indicator tells us how much of the variance in an indicator is explained by the construct
...
Loadings Squared: Indicator Reliability
        COMP  LIKE  CUSA  CUSL
...
NOW COMMENT ON RELIABILITY MEASURES!
...
```
o INTERNAL CONSISTENCY RELIABILITY - where we assess the consistency of results across items for the same construct
...
COMP 0.776 
LIKE 0.831 
CUSA 1.000 (singleitem construct: perfect reliabily)
CUSL 0.831 
 - want indicators having a high correlation among each other within the different indicators summing the constructs.
 - Tends underestimating the internal consistency reliability (conservative measure)
 - The closer to 1 the better – typically around 0.7 - 0.8
 - Should exceed alpha > 0.7

rhoC - Composite Reliability: measures the construct’s reliability.
Reliability:
       rhoC 
COMP  0.865 
LIKE  0.899 
...
o VALIDITY
 - -	Validity refers to the extent a measurement instrument (the indicators in our setting) captures what it is intended to measure (the construct in our setting)
 - keep content or face validity in the back of your mind when you empirically investigate your constructs: Does it make sense to use this set of indicators to measure this construct? Use our own logic to assess the validity as well.

o VALIDITY: CONVERGENT VALIDITY
 ...
 -	Average variance extracted (AVE), is the average amount of variance in the indicators that the construct can explain.
 - AVE > 0.5, means that the construct explains more of the variance in its indicators, than the variance that is left unexplained in the error terms.
...
Alpha, rhoC, and rhoA should exceed 0.7 
AVE should exceed 0.5
These thresholds are met by the measures, which is also seen in the plot.


o VALIDITY: DISCRIMINANT VALIDITY
...
HeteroTrait-MonoTrait (HTMT) ratio
 ...
 - Estimate of what the true correlation would be if the constructs were perfectly measured (i.e., perfectly reliable)
 - should be assessed for any combination of reflectively measured/single item constructs.
```{r Validity HTMT }
# HTMT criterion
...
Bootstrap HTMT output
 - If 1 is inside the confidence interval → indicates lack of discriminant validity (slides)
...
95% Confidence Intervals (CI):
 - confidence intervals (2.5% CI and 97.5% CI) do not include 1 for any of the loadings, reinforcing the significance of these loadings.

Ways to DECREASE HeteroTrait-MonoTrait ratio:
 - removing indicators having low correlation with other indicators of the same construct
 - Reassign indicators to other constructs
 - When removing/changing indicators or merging/splitting constructs, it is very important that this make sense theoretically or conceptually so we preserve content validity



## 10.PLS_IIIa: Formative Measurement model
The necessary steps for an assessment of the formative measurement model
-	Convergent validity
-	Collinearity issues
...
CUSA  1.000 
CUSL  0.748 
 - QUAL, PERF, CSOR, ATTR are MODE B = FORMATIVE MEASURES - CAUTION!!!!!

 - Convergent validity is the extent to which a FORMATIVELY measure correlates positively with other (reflective) measures of the same construct
 -	Average variance extracted (AVE), is the average amount of variance in the indicators that the construct can explain.
 - AVE > 0.5, means that the construct explains more of the variance in its indicators, than the variance that is left unexplained in the error terms.

SUMMARY: 
QUAL has an AVE of 0.518.
PERF has an AVE of 0.488 (slightly below the threshold).
CSOR has an AVE of 0.545.
ATTR has an AVE of 0.540.

Alpha, rhoC, and rhoA should exceed 0.7 overall (but not strictly important in formative model)
AVE should exceed 0.5
These thresholds are met by the measures, which is also seen in the plot.

The Path Coefs are below Redundancy Analysis Section!
...
Convergent Validity:
Convergent validity is the extent to which a formatively measured construct correlates positively with other (reflective) measures of the same construct
 - Path coefficient between Y1 and Y2 at least above 0.7
 - Reflective measures must also be included in the data collection (use scale handbooks to assist in selecting reflective measures)
 - Alternatively use global single item that summarize essence of construct
...
PERF_F  0.811
       ...
       QUAL_G
QUAL_F  0.805
 - Path coefficient between Y1 and Y2 at must exceed 0.7
 - acceptable in this case

o COLLINEARITY ISSUES
- Variance Inflation Factor (VIF): Measures how much the variance of an estimated regression coefficient (weight) increases due to collinearity
- Tolerance (TOL): amount of variance in one formative indicator, not explained by the other indicators from the same construct

Variance Inflation Factor (VIF)
 - Collinearity analysis for several construct values.
 - Measures how much the variance of an estimated regression coefficient (weight) increases due to collinearity
 - VIF measures the extent of multicollinearity among the predictor variables in a regression model. 
  ...
  - VIF > 5 indicates problems with potential collinearity problems
...
SUMMARY:
 - VIF values across all FORMATIVE constructs (QUAL, PERF, CSOR, ATTR) are all well below the threshold of 5, indicating that multicollinearity is not a significant issue in this model.
...
Bootstrapping analysis of a structural equation model (SEM) with formative indicators.
```{r Bootstrap }
...
```
Original Est.: 
These are the original estimated loadings for each indicator on their respective latent constructs (e.g., QUAL, PERF, CSOR).

Bootstrap Mean: 
This is the mean of the bootstrap sample estimates. Bootstrapping involves repeatedly resampling from the data and estimating the parameters for each sample. The Bootstrap Mean is the average of these estimates.

Bootstrap SD: 
This is the standard deviation of the bootstrap sample estimates, indicating the variability of the estimates across the bootstrap samples.

Significant Loadings:
Loadings with high t-statistics and confidence intervals that do not include zero indicate that the indicators are significant contributors to their respective latent constructs.
For example, qual_6 -> QUAL has a high t-statistic of 6.224 and a 95% CI of [0.283, 0.496], indicating a significant and reliable loading.

Non-significant Loadings:
Loadings with low t-statistics and confidence intervals that include zero indicate that the indicators are not significant contributors to their respective latent constructs.
For example, qual_2 -> QUAL has a t-statistic of 0.808 and a 95% CI of [-0.047, 0.124], indicating a non-significant loading.

SUMMARY
Stable and Significant Indicators: qual_6, perf_1, attr_3, etc.
Potentially Non-significant Indicators: qual_2, qual_4, csor_2, etc. - However, all these indicators have loadings above 0.5, so we keep them in the model
Bootstrapping: Provides robust estimates and confidence in the model by resampling and re-estimating the parameters multiple times.



## 10.PLS_IIIb: Structural Model
o	The necessary steps for an assessment of the structural model
	Assessment of collinearity issues and model relationships
	Assessment of explanatory and predictive power
	(Model comparisons)
...
o	STEP 1: Collinearity Assessment
-	Tolerance < 0:2 (VIF > 5) indicates potential collinearity problems
-	If multicollinearity is an issue  consider eliminating constructs or merging predictors into a single construct
...
If high multicollinearity
 - Possible treatment of collinearity issues:	Remove construct or merge predictors into a single construct


o	STEP 2: Assess Relationships
-	Path coefficients have values approximately between -1 and +1
-	Bootstrapping is used to obtain standard errors, t-values, p-values and confidence intervals
-	Identify significant relations
-	Asses if a small but significant effect is relevant
-	Look at direct, indirect and total effects
...
o STEP 3 Explanatory Power
 - R^2 values (look above in the first section after model specification!)
 - Effect size
...
o	The basic concepts of mediation and moderation
o	How to perform a mediation and moderation analysis
o	Understand how to interpret the results of a mediation and moderation analysis
...
Row and Column Variables:
 - The rows represent the independent (exogonous) variables.
 - The columns represent the dependent (endogonous) variables.
...
SUMMARY:
 ...
 - Look in Opslag for intution on direct + total effects and calculations on this.
...
From slides:
o	CUSA fully mediates the relation from COMP to CUSL
o	CUSA partially mediates the relation from LIKE to CUSL – we have complementary mediation because the mediating and direct effect works in the same direction


o MODERATION
 -	A moderator variable changes the strength or the direction of a relation between two constructs. Accounts for heterogeneity in the data
 -	A priori hypothesis about the data: Is it some specific relation that is influenced by the moderator variable (interaction effect), or is it the entire model (multigroup analysis)?
 -	Moderators can be single variables or constructs measured either formatively or reflectively
 -	Can be categorical variables, e.g. the strength of the relation between satisfaction and loyalty could depend on gender. Most often used in multigroup analysis
...
REDUNDANCY: Formative maesurement model
...
SUMMARY:
 - The negative moderation effect implies that as SC (possibly a context or condition) increases, the positive impact of customer satisfaction (CUSA) on customer loyalty (CUSL) decreases
...
From slides:
o	Based on existing theory the Switching Cost (SC) construct is introduced as a moderator of the relationship between satisfaction and loyalty (reflectively measured)
o	The respecified measurement models do fulfill the necessary requirements
o	The interaction term (CUSA  SC) is significant with a negative sign whereas the associated simple effect of CUSA on CUSL is positive – the latter relationship is thus moderated by SC


##Influencer model PLS-SEM

```{r}
#REFLECTIVE MEASUREMENTS MODELS HTMT
# R script for the illustrations in chapter 4
rm(list=ls())
library(seminr)
library(readxl)
...
## Preparation
# Load data and inspect the first few observations
#influencer_data <- read.csv("PLS_data_exam.csv")
library(readr)
...
## Estimation
# Estimate the model
influencer_pls_model <- estimate_pls(data = influencer_data,
                                     measurement_model = simple_mm,
                                     structural_model = simple_sm,
                                     inner_weights = path_weighting,
                                     missing = mean_replacement,
